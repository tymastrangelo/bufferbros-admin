-- 0017: boat pricing components, skippable plan entry detail, plan-projection availability.
--   * Boat Detail splits into per-foot components — exterior wash + spray wax make the
--     outside rate, interior stacks on top. Each rate edits independently in Settings;
--     a boat quote sums the components × length.
--   * plans.skip_entry — a plan can skip the discounted entry detail and start straight
--     at the maintenance rate.
--   * plan_projection_busy — an active plan's future visits hold their slot forever,
--     even beyond the materialized horizon: get_available_slots and overlap-mode
--     book_appointment treat every projected date+time as busy ('force' still wins).

-- ============ BOAT COMPONENTS ============
insert into service_pricing (service_id, size_id, price, minutes) values
  ('boat-detail', 'wash-ft',     10, 3),   -- exterior wash
  ('boat-detail', 'wax-ft',       5, 2),   -- spray wax
  ('boat-detail', 'interior-ft',  8, 3)    -- interior
on conflict (service_id, size_id) do nothing;
delete from service_pricing where service_id = 'boat-detail' and size_id = 'per-ft';
update services
  set note = 'Exterior wash + spray wax by the foot; interior per foot on top. Every quote is still adjustable per job.'
  where id = 'boat-detail';

-- ============ SKIPPABLE ENTRY DETAIL ============
alter table plans add column if not exists skip_entry boolean not null default false;

-- ============ PLAN PROJECTION ============
-- Does any active plan's *projected* (not yet materialized) visit overlap
-- [p_start, p_end) on p_date? Mirrors src/lib/plan-projection.ts and
-- generateOccurrences: continue from the last non-cancelled visit in step-day
-- increments; with no visits yet, start at max(starts_on, today) advanced to the
-- preferred weekday. A date that already has an appointment row for the plan (even
-- cancelled) was handled by a human and is not projected. Plans without a preferred
-- time can't hold a slot. p_exclude_plan lets a plan materialize into its own slot.
create or replace function plan_projection_busy(
  p_date date, p_start int, p_end int, p_buffer int, p_exclude_plan uuid default null
) returns boolean
language plpgsql stable
set search_path = public
as $$
declare
  r record;
  v_tz    text := coalesce((select value from settings where key = 'timezone'), 'America/New_York');
  v_today date;
  v_step  int;
  v_anchor date;
begin
  v_today := (now() at time zone v_tz)::date;
  if p_date < v_today then return false; end if;

  for r in
    select p.id, p.cadence, p.interval_days, p.preferred_dow, p.starts_on, la.last_date
    from plans p
    left join lateral (
      select max(a.date) as last_date
      from appointments a
      where a.plan_id = p.id and a.status <> 'cancelled'
    ) la on true
    where p.status = 'active'
      and p.preferred_min is not null
      and (p_exclude_plan is null or p.id <> p_exclude_plan)
      and p.starts_on <= p_date
      and (p.ends_on is null or p_date <= p.ends_on)
      and p_start < p.preferred_min + p.duration_min + p_buffer
      and p_end > p.preferred_min
  loop
    -- any appointment on this exact date (even cancelled) = human handled it
    continue when exists (select 1 from appointments a where a.plan_id = r.id and a.date = p_date);

    v_step := case r.cadence
      when 'weekly' then 7 when 'biweekly' then 14 when 'monthly' then 28
      else coalesce(r.interval_days, 28) end;

    if r.last_date is not null then
      if p_date > r.last_date and (p_date - r.last_date) % v_step = 0 then return true; end if;
    else
      v_anchor := greatest(r.starts_on, v_today);
      if r.preferred_dow is not null then
        v_anchor := v_anchor + ((r.preferred_dow - extract(dow from v_anchor)::int + 7) % 7);
      end if;
      if p_date >= v_anchor and (p_date - v_anchor) % v_step = 0 then return true; end if;
    end if;
  end loop;
  return false;
end $$;

-- ============ AVAILABILITY (0016 + projection check) ============
create or replace function get_available_slots(
  p_date         date,
  p_duration_min int,
  p_employee_ids uuid[] default null,
  p_require_all  boolean default true
)
returns table (slot_min int)
language plpgsql stable
set search_path = public
as $$
declare
  v_tz       text;
  v_gran     int;
  v_lead     int;
  v_buffer   int;
  v_enabled  boolean;
  v_open     int;
  v_close    int;
  v_local    timestamp;
  v_today    date;
  v_earliest int := 0;
  v_total    int;
  v_ids      uuid[] := p_employee_ids;
  v_all      boolean := coalesce(p_require_all, true);
begin
  select coalesce((select value from settings where key='timezone'), 'America/New_York') into v_tz;
  select coalesce((select value::int from settings where key='slot_granularity_min'), 30) into v_gran;
  select coalesce((select value::int from settings where key='min_lead_min'), 180) into v_lead;
  select coalesce((select value::int from settings where key='buffer_min'), 30) into v_buffer;

  -- Public/no-target path: honor the website worker toggle when set.
  if v_ids is null then
    begin
      v_ids := nullif(string_to_array(coalesce((select value from settings where key='web_slot_employee_ids'), ''), ','), '{}')::uuid[];
    exception when others then
      v_ids := null;  -- unparseable setting: fall back to the whole-business calendar
    end;
    v_all := false;    -- website: a slot works if ANY checked worker can take it
  elsif array_length(v_ids, 1) is null then
    v_ids := array[null::uuid];  -- explicit {} = the owner
  end if;

  select enabled, open_min, close_min into v_enabled, v_open, v_close
  from weekly_hours where weekday = extract(dow from p_date)::int;
  if not found or not v_enabled then return; end if;

  v_local := now() at time zone v_tz;
  v_today := v_local::date;
  if p_date < v_today then return; end if;
  if p_date = v_today then
    v_earliest := extract(hour from v_local)::int * 60 + extract(minute from v_local)::int + v_lead;
  end if;

  v_total := p_duration_min + v_buffer;

  return query
  select s
  from generate_series(v_open, v_close - p_duration_min, v_gran) as s
  where s >= v_earliest
    -- a projected plan visit is unassigned, so it blocks everyone on every path
    and not plan_projection_busy(p_date, s, s + v_total, v_buffer)
    and (
      -- legacy whole-business calendar: every block + every scheduled job is busy
      (v_ids is null and not exists (
        select 1 from (
          select b.start_min as bs, b.end_min as be
          from blocks b where b.date = p_date
          union all
          select a.start_min, a.start_min + a.duration_min + v_buffer
          from appointments a where a.date = p_date and a.status = 'scheduled'
        ) busy
        where s < busy.be and s + v_total > busy.bs
      ))
      -- targeted: ALL listed free (multi-worker job) / ANY listed free (website)
      or (v_ids is not null and v_all
          and not exists (select 1 from unnest(v_ids) e where emp_window_busy(p_date, s, s + v_total, e, v_buffer)))
      or (v_ids is not null and not v_all
          and exists (select 1 from unnest(v_ids) e where not emp_window_busy(p_date, s, s + v_total, e, v_buffer)))
    )
  order by s;
end $$;

-- ============ BOOKING (0016 + projection check in overlap mode) ============
create or replace function book_appointment(
  p_date         date,
  p_start_min    int,
  p_duration_min int,
  p_name         text default null,
  p_email        text default null,
  p_phone        text default null,
  p_address      text default null,
  p_size_id      text default null,
  p_size_label   text default null,
  p_service_name text default 'The Standard Detail',
  p_addons       jsonb default '[]',
  p_price        numeric default 0,
  p_notes        text default null,
  p_source       appointment_source default 'web',
  p_customer_id  uuid default null,
  p_vehicle_id   uuid default null,
  p_plan_id      uuid default null,
  p_mode         text default 'strict',
  p_employee_ids uuid[] default null   -- assignees; {} = the owner, null = whole-business check
) returns appointments
language plpgsql
set search_path = public
as $$
declare
  v_row appointments;
  v_buffer int;
  v_ids uuid[] := p_employee_ids;
  v_first uuid;
  v_conflict boolean;
begin
  perform pg_advisory_xact_lock(hashtext('book:' || p_date::text));
  if p_mode = 'strict' and not exists (
    select 1 from get_available_slots(p_date, p_duration_min, p_employee_ids, true) where slot_min = p_start_min
  ) then
    raise exception 'slot_taken' using hint = 'Sorry, that time was just taken. Please pick another slot.';
  end if;
  if p_mode = 'overlap' then
    select coalesce((select value::int from settings where key='buffer_min'), 30) into v_buffer;
    if v_ids is not null and array_length(v_ids, 1) is null then v_ids := array[null::uuid]; end if;
    if v_ids is null then
      -- whole-business check, exactly as before (recurring generation uses this)
      select exists (
        select 1 from (
          select b.start_min as bs, b.end_min as be from blocks b where b.date = p_date
          union all
          select a.start_min, a.start_min + a.duration_min + v_buffer
          from appointments a where a.date = p_date and a.status = 'scheduled'
        ) busy
        where p_start_min < busy.be and p_start_min + p_duration_min + v_buffer > busy.bs
      ) into v_conflict;
    else
      -- every assignee must be free
      select exists (
        select 1 from unnest(v_ids) e
        where emp_window_busy(p_date, p_start_min, p_start_min + p_duration_min + v_buffer, e, v_buffer)
      ) into v_conflict;
    end if;
    -- other plans' projected visits hold their slots; a plan may fill its own
    if not v_conflict then
      v_conflict := plan_projection_busy(p_date, p_start_min, p_start_min + p_duration_min + v_buffer, v_buffer, p_plan_id);
    end if;
    if v_conflict then
      raise exception 'slot_taken' using hint = 'That time overlaps another job, a block, or a plan''s standing slot.';
    end if;
  end if;

  select e into v_first from unnest(p_employee_ids) e where e is not null limit 1;

  insert into appointments
    (date, start_min, duration_min, size_id, size_label, service_name, addons, price,
     address, contact_name, contact_phone, contact_email, notes, source,
     customer_id, vehicle_id, plan_id, employee_id)
  values
    (p_date, p_start_min, p_duration_min, p_size_id, p_size_label, p_service_name,
     coalesce(p_addons, '[]'::jsonb), coalesce(p_price, 0),
     p_address, p_name, p_phone, p_email, p_notes, p_source,
     p_customer_id, p_vehicle_id, p_plan_id, v_first)
  returning * into v_row;

  insert into appointment_employees (appointment_id, employee_id)
    select v_row.id, e from unnest(p_employee_ids) e where e is not null
    on conflict do nothing;

  return v_row;
end $$;

revoke execute on function plan_projection_busy(date, int, int, int, uuid) from public, anon;
